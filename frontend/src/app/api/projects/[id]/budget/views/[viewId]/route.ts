...
export async function GET(
  ...
  try {
    ...
    console.error('Error in GET /api/projects/[id]/budget/views/[viewId]:', error);
    return NextResponse.json(
  ...
  try {
    ...
    if (name || description !== undefined || is_default !== undefined) {
      ...
      if (updateError) {
        ...
        return NextResponse.json(
          { error: 'Failed to update budget view', details: updateError.message },
          { status: 500 }
        );
      }
    }
    ...
    if (columns && columns.length > 0) {
      ...
      if (deleteError) {
        ...
        return NextResponse.json(
          { error: 'Failed to update budget view columns', details: deleteError.message },
          { status: 500 }
        );
      }
      ...
      if (insertError) {
        ...
        return NextResponse.json(
          { error: 'Failed to update budget view columns', details: insertError.message },
          { status: 500 }
        );
      }
    }
    ...
    if (fetchUpdatedError) {
      ...
      return NextResponse.json(
        { error: 'Failed to fetch updated budget view', details: fetchUpdatedError.message },
        { status: 500 }
      );
    }
    ...
    const viewWithSortedColumns = {
      ...updatedView,
      columns: updatedView.columns?.sort((a, b) => a.display_order - b.display_order) || [],
    };

    return NextResponse.json({ view: viewWithSortedColumns });
  } catch (error) {
    console.error('Error in PATCH /api/projects/[id]/budget/views/[viewId]:', error);
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    );
...
export async function DELETE(
  ...
  try {
    ...
    console.error('Error in DELETE /api/projects/[id]/budget/views/[viewId]:', error);
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    );
