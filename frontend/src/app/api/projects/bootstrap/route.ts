...
const WAREHOUSE_TEMPLATE = {
  ...
  contract: {
    ...
    scopeOfWork: 'Complete construction of 50,000 sq ft warehouse facility including foundation, structure, MEP systems, and finishes.',
  },
  ...
  commitment: {
    ...
    scopeOfWork: 'Supply and install all concrete work including slab-on-grade, walls, and elevated decks.',
  },

  changeEvent: {
    title: 'Owner Requested Storage Expansion',
    description: 'Owner requests additional 5,000 sq ft storage area in northwest corner',
  ...
  changeOrder: {
    ...
    description: 'Additional structural steel, concrete, and roofing for 5,000 sq ft expansion',
    amount: 125000,
  }
};
...
export async function POST(request: Request) {
  try {
    ...
    const missingCostCodes = projectTemplate.costCodes.filter(cc => !existingIds.has(cc.id));
    ...
    if (missingCostCodes.length > 0) {
      ...
      if (ccError && !ccError.message.includes('duplicate')) {
        ...
        return NextResponse.json({ error: `Failed to create cost codes: ${ccError.message}` }, { status: 500 });
      }
    }
