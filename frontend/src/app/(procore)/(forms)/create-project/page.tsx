...
import { useForm, type ControllerRenderProps, type DefaultValues } from 'react-hook-form';
import { z } from 'zod';
...
const optionalNumeric = z.preprocess(numberFromInput, z.number().nonnegative().optional());
...
const createProjectSchema = z.object({
  ...
  erp_sync: z.boolean().default(true),
  test_project: z.boolean().default(false),
});

type CreateProjectFormValues = z.infer<typeof createProjectSchema>;
type FieldName = keyof CreateProjectFormValues;

type FieldControl = 'text' | 'textarea' | 'select' | 'number' | 'date' | 'checkbox' | 'file';
...
const formSections: FormSection[] = [
    ...
    fields: [
      {
        ...
        label: 'Active',
        control: 'checkbox',
        description: 'Default checked — active projects appear across the company.',
        colSpan: 'full',
      },
    ],
  },
  ...
  },
  {
    ...
    description: 'Flag projects that will sync once ERP automation ships (not yet live).',
    fields: [
      {
        name: 'erp_sync',
        label: 'ERP-sync this project',
        control: 'checkbox',
        description: 'Indicator only for now — ERP sync services are still in development.',
        statusHint: 'Not live',
        colSpan: 'full',
      },
    ],
  },
  {
    id: 'advanced',
    title: 'Advanced',
    description: 'Use for sandbox tagging — feature is informational only until automation is wired.',
    fields: [
      {
        name: 'test_project',
        label: 'Test Project',
        control: 'checkbox',
        description: 'Marks the record as sandbox/test (reports do not yet auto-filter on this flag).',
        statusHint: 'Not live',
        colSpan: 'full',
      },
    ],
...
export default function CreateProjectPage() {
  return (
    <AppShell companyName="Alleato Group" currentTool="Portfolio" userInitials="BC">
      <div className="flex min-h-[calc(100vh-48px)] flex-col bg-gray-50">
        ...
        if (field.control === 'checkbox') {
          return (
            <FormItem className={baseClasses}>
              <div className="flex items-start space-x-3 rounded-md border border-input bg-white px-4 py-3">
                <FormControl>
                  <Checkbox
                ...
                <div className="space-y-1">
                  <FormLabel htmlFor={String(field.name)} className="font-medium leading-none">
      ...
      <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
        {formSections.map((section) => (
          <Card key={section.id}>
            ...
            <CardContent>
              <div className="grid grid-cols-1 gap-5 md:grid-cols-2">{section.fields.map(renderField)}</div>
            </CardContent>
          </Card>
        ))}

        <div className="flex flex-col gap-3 rounded-lg border border-dashed border-muted bg-white p-4 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="text-sm font-medium text-gray-900">Need to start over?</p>
            <p className="text-sm text-muted-foreground">Reset clears unsaved values but keeps Procore defaults.</p>
          </div>
