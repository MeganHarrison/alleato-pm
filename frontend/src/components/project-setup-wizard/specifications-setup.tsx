...
export function SpecificationsSetup({ projectId, onNext, onSkip }: StepComponentProps) {
  ...
  const onDrop = useCallback(async (acceptedFiles: File[]) => {
    ...
    for (const uploadFile of newFiles) {
      try {
        ...
        const filePath = `projects/${projectId}/specifications/${uploadFile.id}.${fileExt}`
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        <p className="text-muted-foreground">
          Upload project specifications and technical documents. You can add more specifications anytime after setup.
        </p>
          ...
          {isDragActive ? (
            ...
            <>
              <p className="text-lg font-medium mb-1">
                Drag & drop specifications here, or click to select
              </p>
              <p className="text-sm text-muted-foreground">
                PDF, Word, Text (Max 50MB per file)
              </p>
            </>
          )}
        </div>
