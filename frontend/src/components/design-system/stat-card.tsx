...
export function StatCard({ label, value, icon: Icon, trend, onClick, href }: StatCardProps) {
  const baseClasses = "border border-neutral-200 bg-white p-8 transition-all duration-300 hover:border-[#DB802D] hover:shadow-sm"
    ...
    <>
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          {Icon && <Icon className="h-4 w-4 text-[#DB802D]" />}
        </div>
