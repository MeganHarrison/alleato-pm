...
interface UseChangeOrdersReturn {
  ...
  createChangeOrder: (changeOrder: Partial<ChangeOrder>) => Promise<ChangeOrder | null>
}
 ...
 */
export function useChangeOrders(options: UseChangeOrdersOptions = {}): UseChangeOrdersReturn {
  const { contractId, projectId, commitmentId, status, limit = 100, enabled = true } = options
  ...
  const fetchChangeOrders = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err : new Error('Failed to fetch change orders'))
    } finally {
      setIsLoading(false)
    }
  ...
  const createChangeOrder = useCallback(async (changeOrder: Partial<ChangeOrder>): Promise<ChangeOrder | null> => {
    try {
      const supabase = createClient()
      ...
      setError(err instanceof Error ? err : new Error('Failed to create change order'))
      return null
    }
  }, [fetchChangeOrders])
 ...
 */
export function useContractChangeOrders(contractId: number, projectId?: number) {
